...
class DataOpener:
    ...
    def get_data(self):
        ...
        self.isFrequencySelection = data_mdf['/measurement/isFrequencySelection'][()] # flag for frequency selection
    ...
    def reorder_separatebackground(self):
        ...
        if self.framePermutation != None:
            ordermask = self.framePermutation -1
            # reorder
            self.data = self.data[ordermask,:,:,:]
            self.isBackgroundFrame = self.isBackgroundFrame[ordermask]
    ...
    def reorder_separatebackground2(self):
        # select background scans
        backgroundmask = self.isBackgroundFrame.astype(bool)
        foreground_mask = np.logical_not(self.isBackgroundFrame)
        self.background_data = self.data[backgroundmask,:,:,:]
